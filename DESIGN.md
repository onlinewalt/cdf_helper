...
- **表头自动识别**（`_find_header_row`）：
  ...
  2. 未精确命中时按 `_CONTAINS_RULES` 包含匹配（优先级：名称 > 规格/型号 > 数量 > 单位 > 重量 > 单价）。新增 `description`→名称、`q'ty`→数量、`part no`→规格的包含规则。
   3. 当某行同时识别到 `name` 与 `qty` 即视为表头行。（`_CONTAINS_RULES` 循环不再 `break`，以支持单单元格含多个表头关键字的紧凑格式）
- **紧凑单列格式**（`_parse_packed_sheet`）：当所有表头字段映射到同一列（如 `Sheet2`/`Sheet6` 等）时，进入紧凑格式解析——
  1. 表头行单元格可能包含公司信息 + 表头 + 数据行（用换行分隔），或仅表头。
  2. 按 2+ 空格分割表头字段：`序号 / 设备 / 名称 / 编号 / 单位 / 数量 / 备注`。
  3. 数据行同样按 2+ 空格分割，首个数字为序号（跳过），最后一个数字为数量。
  4. 中间值按位置分配给 `设备 / 名称 / 编号`：含中文的为设备，含 `× / /Dwg / Code / Drg` 模式的为编号（type），其余为名称。
  5. 缺少字段（如单位、设备）时自动跳过，页脚行（目的地/签字/日期等）被过滤。
  6. 适配多行单元格格式（Sheet4/5/9/12：表头与数据同单元格换行分隔）和多行格式（Sheet2/6/8：表头与数据在不同行）。
