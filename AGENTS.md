...
- **Workbook fails to open / every sheet missing.** The `.xlsx` adapter (`WorkbookCache._load`) applies a scoped lenient cast over `openpyxl.worksheet._reader._cast_number` (via the `_lenient_openpyxl_cast` context manager) so a non-numeric value in a number-typed cell (e.g. a stray `.`) returns a string instead of raising `ValueError: could not convert string to float`, which would otherwise abort the whole load. The patch is saved & restored within the `with` block, so it does **not** leak to unrelated openpyxl usage outside the load; the old process-global `cache_openpyxl_lenient` mutation is gone, and sheet construction (`_OpenpyxlSpreadsheetSheet.__init__`) no longer mutates openpyxl. A strict adapter is available via `WorkbookCache(lenient=False)`. Workbooks are opened through the injectable `WorkbookCache` (bounded LRU, `max_entries=8`) rather than the old module-global `_sheets_cache`; `.xls` goes through `xlrd` (`_XlrdSpreadsheetSheet`). If the crash still occurs, the openpyxl internals changed and `_lenient_openpyxl_cast` needs updating.
...
Each `test_*.py` test below locks down one historical failure; if a real file regresses, reproduce it hermetically with an `openpyxl.Workbook()` helper (pattern: `_write_*_workbook` + `assert len(parse_source(...)) == N`):
  ...
  - `test_bad_number_cell_xlsx_loads` — corrupt numeric cell no longer aborts the `.xlsx` load (`_lenient_openpyxl_cast` inside `WorkbookCache._load`).
